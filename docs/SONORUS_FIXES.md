# Sonorus — several bugs found during a fresh OmniVoice setup (with fixes)

First, let me say thanks for building Sonorus! It's a fantastic mod, but I ran into a few
different issues while trying to get it set up and running flawlessly. I used Claude to
investigate and fix the bugs I found, and below is a write-up written by Claude. If there's
anything you want changed/more info on, let me know. I'm also happy to file a separate
PR for each of these if you'd like.

> **Note on format:** This is intentionally **one combined issue** covering several
> distinct bugs, rather than seven separate issues, so I don't spam the issues.
...
> done by Claude while helping me get Sonorus running on a
> new machine. Each fix was applied to a local copy and syntax-checked
> (`py_compile`); the input-hook (#7) fix was also verified in-game.
> Please review before merging.
>
> **PRs:** I'm happy to open a **separate PR for each** of these if that's useful.
> Just let me know which ones you'd want.
>
> **Already reported:** Number 1 below is a repeat of Issue #2 — it's included only for
> completeness, since it was part of the same setup run.
>
> **Version note:** Originally found on 1.0.7; I re-confirmed all of the below are **still
> present in 1.0.8 pre-release 4**. I dropped two earlier findings that you've *already*
> fixed in 1.0.8 — the `current_llm_provider` NameError (pre-2) and the Pocket TTS
> voice-overlap (pre-3) — so they're not listed here.

## Environment
- **GPU:** NVIDIA GeForce RTX 5080 (16 GB)
- **Driver:** 610.62 (Game Ready), reports `CUDA UMD Version: 13.3`
- **OS:** Windows (Build 26200, 25H2)
- **Sonorus:** 1.0.7, re-verified on 1.0.8 pre-release 4
...
| 4 | OmniVoice deps | torchcodec needs FFmpeg 4–7 on Windows (FFmpeg 8 silently fails) | `install_omnivoice.bat` |
...
| 7 | Input | Chat keyboard hook gets culled by Windows (typing load + fast-travel); dies until server restart | `input/text.py`, `server.py`, `utils/lua_socket.py` |
---
...
```
---
## 4. torchcodec needs FFmpeg 4–7 on Windows (FFmpeg 8 silently fails)
**Symptom:** After deps installed, playback failed with
`ModuleNotFoundError: No module named 'torchcodec'`; after installing torchcodec, a
`TorchCodec / FFmpeg` error.
**Cause:** `torch>=2.4.0` installs the newest torch/torchaudio (2.11 here), whose
`torchaudio.load()` routes through `torchaudio/_torchcodec.py::load_with_torchcodec`,
which requires **torchcodec**, and torchcodec requires **FFmpeg** shared libraries.
1.0.8 pre-4 now installs torchcodec in the OmniVoice step, so the
`ModuleNotFoundError` is resolved — but the FFmpeg requirement remains: on Windows,
torchcodec supports FFmpeg **4–7 only** (FFmpeg 8 is Mac/Linux), and those DLLs aren't
bundled or documented, so a "latest FFmpeg" build (v8) is silently not detected and
audio load still fails.
...
- Keep torchcodec but document/bundle **FFmpeg 7** shared DLLs
...
---
## 7. Text-input keyboard hook gets culled by Windows and never recovers
