...
> **Branch note:** This development branch is based on the 1.0.8 pre-release 5
> source and adds a native OmniVoice (Vulkan) provider. See
> [the branch architecture overview](docs/BRANCH_ARCHITECTURE_OVERVIEW.md) for
> the integration design and differences from the base mod.
...
- Who else is nearby
- Whether you're invisible, in combat, on a broom or mount, or swimming
...
- Completed quest facts and active commitments
- Supported NPC schedule mod context
- Character static bios and guidance
- World lore (configurable static info all NPCs share)

**Group Conversations**

When multiple NPCs are nearby, they can react to your conversations and speak to each other. Discussions can flow naturally between characters, with NPCs looking toward whoever is speaking.

**NPC Followers**

NPCs can follow you as a group when enabled in Conversation → Experimental. Followers can be dismissed from the web configuration page, are more likely to join conversations, and can teleport back when they get stuck or fall too far behind.

**Companion Commentary**

Companions can occasionally make unprompted remarks after notable events. This is optional and can be disabled in settings.

**NPC Commitments**

NPCs can agree to meet at 65+ locations across Hogwarts and Hogsmeade, then travel there and wait for you (up to 45 in-game minutes). If you don't show up, they'll remember the no-show.

**Owl Post & Board**

Send and receive Owl Mail in-game, including commitment reminders, custom Owl Mail characters, Howlers, and Board posts. Owl Post has configurable active hours, daily limits, hotkeys, and controls for resetting or deleting mail data.
...
Use first-person mode manually (default hotkey: Insert), or let conversations automatically transition into a first-person cinematic look-at mode. These options are configurable in Conversation → Experimental.

**Gaze & Facial Expressions**

NPCs can meet your gaze, start or continue conversations when you look at them nearby, and use facial expressions during dialogue. These immersion features are configurable in Conversation → Experimental.

**VR Support (UEVR)**

Sonorus supports VR through UEVR with headtracking 3D audio, gaze offset, and gesture controls. Tap grip near your mouth to toggle Open Mic, or hold grip to stop a conversation. The VR plugin installer can download the Sonorus VR Bridge on demand and includes optional best-setting helpers.
...
An optional narrator speaks in third person between NPC dialogue, describing actions, expressions, and scene transitions. Disabled by default — enable it in Conversation → Experimental.
...
Sonorus keeps conversations coherent in the moment while preserving long-term continuity across play sessions.
...
During live conversations, only a configurable amount of the most recent dialogue is included in the AI’s context. This keeps responses fast, relevant, and focused while still preserving the full history for reference and management.

**Long-Term Memory**

NPCs can build simpler long-term memories from important moments, relationships, promises, and shared history. During future conversations, Sonorus searches for relevant memories and adds them to context so characters can remember what matters without replaying entire chat logs.

Memory is stored separately for each player character. The first character you load after updating will inherit your existing Sonorus data; other player characters start with their own fresh data folder.

The web configuration page includes controls for memory backup, restoration, migration, whitelisting, static character bios, and embedding model selection. Sonorus will also warn if you load a save whose game time appears older than your stored Sonorus data.
...
- **Ollama Cloud** - AI responses from small free cloud models
- **Voice Synthesis:**
    - **Pocket TTS** - Local voice cloning (runs on your PC, English only)
    - **OmniVoice** - Local voice cloning (runs on your PC, requires about 2 GB VRAM available, supports all game languages)
    - **OmniVoice (Vulkan)** - Native local voice cloning for Vulkan-capable AMD, Intel, or NVIDIA GPUs, with a CPU fallback and 48 kHz output; it does not require PyTorch or CUDA
    - **Inworld** - Cloud TTS ($2 free credit, about 45 minutes of audio; up to 100 voice clones on free and pay-as-you-go plans)
- **Speech-to-Text:**
...
**Note:** Gemini has reduced their free tier rate limits recently. If you hit daily limits, try Ollama Cloud for small models or [OpenRouter](https://openrouter.ai/) - a $5 minimum deposit will usually last a long time.

## Other Services

You can use other services, proxies, or locally hosted models:
- **OpenRouter** - AI responses (Recommended! Minimum $5 deposit)
- **OpenAI** - AI responses (Official API or a proxy)
- **llama.cpp** - Local LLM provider with KV cache save/restore support (requires an unused GPU or spare PC with GPU that has 12-24GB VRAM)
- **Voice Synthesis:**
...
1. Make sure Hogwarts Legacy is closed
1. Double-click
    `install_sonorus.bat`
    and follow the prompts
1. Launch Hogwarts Legacy
1. A setup window will open in the background - wait for it to complete
1. Your web browser will open with the configuration wizard
1. Complete the wizard to configure your API keys and settings
1. Enable subtitles in your Hogwarts Legacy settings (for chat input to be visible)
1. Use the configured hotkeys or your microphone to talk to NPCs
...
Run
`install_sonorus.bat`
again from a newer version of the zip. Your data (memories, dialogue, settings) will not be affected.

**Important for 1.0.7:** Sonorus now stores data separately for each player character. The first character you load after updating will inherit your existing Sonorus data; other player characters start with their own fresh data folder.

**Long-term memory users:** 1.0.7 uses a simpler long-term memory system. If you use this feature, reset the memory system and re-migrate NPCs from the web configuration page.

**Potential Issue:** I am having trouble with voice clones on Inworld after upgrading.
**a)** Inworld currently allows up to 100 voice clones on the free and pay-as-you-go plans. If you hit the limit, Sonorus can automatically delete older generated clones so new voices can still be created.

## Uninstalling

Run
`uninstall_sonorus.bat`
from the same zip you used to install. It will offer to back up your data (memories, dialogue, settings) before removing anything.
...
- Internet connection for cloud AI services and first-time optional downloads
- About 2 GB available VRAM (after Hogwarts Legacy is running) if using OmniVoice

## Compatibility

- **Languages**: Voice manifests are currently included for English, German, Italian, Brazilian Portuguese, Latin American Spanish, Spanish (Spain), French, and Japanese. OmniVoice supports all game languages, including undubbed languages such as Arabic, Polish, Russian, Korean, Simplified Chinese, and Traditional Chinese. Additional voices and languages can be added manually using the /voice-manager/ in the web interface.
...
- **Other UE4SS mods**: Should work, but avoid multiple UE4SS installations. If another mod includes UE4SS, manually install that mod into ue4ss/Mods after installing Sonorus.
...
- **NPC Schedules Enhanced**: Supported - schedule context and schedule changes are shared with NPCs
...
- NPCs sometimes to not show up to their commitments.
...
- **Stability & Performance**: Broad stability improvements across game load, save load, fast travel, memory processing, conversation state, and server worker paths
- **Owl Post & Board**: Add Owl Mail, Board posts, Howlers, custom Owl Mail characters, commitments in mail, active hours, hotkeys, daily limits, context improvements, and reset/delete controls
- **NPC Followers**: Add group followers, companion/follower panels with dismiss buttons, stuck/far-away teleport recovery, better follower speaker selection, and companion commentary after notable events
- **First-Person Conversations**: Add first-person mode, conversation-triggered first-person, cinematic look-at options, mount handling fixes, and configurable transition duration
- **Gaze & Expressions**: Add gaze-initiated conversations, NPCs meeting gaze, speaker-facing head/eye orientation, and expanded facial expressions during dialogue
- **Memory**: Add a simpler long-term memory system with backup/restore, whitelisting, static character bios, embedding model selection, migration tools, save-aware player-character data folders, out-of-sync warnings, and corruption fixes
- **Commitments & World Context**: Expand commitments to 65+ locations, add guidance paths to active commitments, improve commitment reliability, and add completed quest facts to NPC context
- **LLM Providers**: Add Ollama Cloud provider, add llama.cpp provider with KV cache save/restore, update model defaults, add character-specific chat model overrides, improve cache hits, and support new Gemini API key prefixes
- **TTS & Voice**: Add OmniVoice local TTS with GPU selection and smoothing EQ, add Spanish (Spain) and Japanese voice manifests, support undubbed languages, improve voice activity detection, and automatically handle Inworld voice clone limits
- **Web Config**: Add Simple Mode, search, ID auto-complete, OpenRouter model auto-complete, cost tracking, archived TTS playback, better setup errors, hotkey display, mod chunk ID conflict warnings, and hold-Escape in-game overlay access
- **VR**: Add HMD Audio Fix integration, on-demand Sonorus VR Bridge download, consolidated VR settings, and optional best-settings toggle
- **Auto-Mute & Ambient Dialogue**: Add repeated ambient dialogue auto-mute, improve exclusions for ghosts/portraits/shopkeepers, and fix mute/unmute behavior when Auto-Mute is disabled
- **Conversation Polish**: Add farewell lines, EndConversation NPC action, follow-up nudges, narration in player messages, rhetorical question filtering, better JSON repair, portrait conversations, and improved prompt handling
- **Installer & Dependencies**: Fix installer/uninstaller issues, improve pre-existing UE4SS handling, add enable/disable .bat files, and pin Python dependencies to strict versions
...
**a)** The "Phoenix\Binaries\Win64\sonorus\data" folder contains dialogue history, memory, settings, and player-character data folders. Copy this folder somewhere secure to back it up. The web configuration page also includes memory backup and restore tools. Alternatively,
`uninstall_sonorus.bat`
will offer to back up your data before removing the mod.
...
**a)** There is a toggle in Server section of the mod configuration page, or you can use the enable/disable .bat files included with the installer.

**8.** I hit Gemini free tier quota. What do I do?
**a)** Try Ollama Cloud for small free models, or use OpenRouter - it has a minimum $5 deposit and often goes a long way. Otherwise, you can wait until the next day when your rate limit resets. Gemini Pro and other subscriptions that do not include an API are not supported.

**9.** The Sonorus server or configuration page does not open when I start the game.
**a)** Try re-running
`install_sonorus.bat`
to ensure everything was installed correctly. If the issue persists, delete the
`ue4ss`
folder and
`dwmapi.dll`
from your Phoenix/Binaries/Win64 directory and run the installer again.

**10.** Can I use a local LLM or a subscription that includes an API like NanoGPT?
**a)** Yes. You can use an OpenAI compatible API by setting the provider to OpenAI and adding your own endpoint URL, or use the built-in llama.cpp provider for local models. Ollama Cloud is also available for small free cloud models. If you're using a local model, most smaller LLMs (<70B params) do not have enough world knowledge to portray the characters accurately.
...
**15.** Voice spells keep getting transcribed wrong when I talk.
**a)** Try switching to Deepgram for speech recognition - it has better spell keyword detection compared to the local options.

**16.** I am having trouble with voice clones on Inworld after upgrading.
**a)** Inworld currently allows up to 100 voice clones on the free and pay-as-you-go plans. If you hit the limit, Sonorus can automatically delete older generated clones so new voices can still be created.
...
- [NVIDIA Parakeet](https://huggingface.co/nvidia/parakeet-tdt-0.6b-v3) [ONNX](https://huggingface.co/istupakov/parakeet-tdt-0.6b-v3-onnx) (CC-BY-4.0)
- [NVIDIA Canary 180M Flash](https://huggingface.co/nvidia/canary-180m-flash) [ONNX](https://huggingface.co/istupakov/canary-180m-flash-onnx) (CC-BY-4.0)
- [Moonshine](https://huggingface.co/UsefulSensors/moonshine) [ONNX](https://huggingface.co/onnx-community/moonshine-base-ONNX) (MIT)

**Voice synthesis:**

- [Kyutai Pocket TTS](https://github.com/kyutai-labs/pocket-tts) [ONNX](https://huggingface.co/KevinAHM/pocket-tts-onnx) (CC-BY-4.0)
