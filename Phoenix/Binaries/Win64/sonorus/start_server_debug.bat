...
"%PYTHON%" -c "import sys; import importlib.util; sys.exit(0 if all(importlib.util.find_spec(m) is not None for m in ['onnx_asr', 'websocket', 'diskcache', 'posthog', 'dotenv', 'kaldi_native_fbank']) else 1)" >nul 2>&1
if errorlevel 1 (
