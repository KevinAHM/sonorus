"""
Conversation Mode Cycling Hotkey

Configurable hotkey that cycles through conversation modes:
- default: NPCs can interject, max 6 turns (standard behavior)
- 1to1: No NPC interjections
- continuous: No turn limit (unlimited interjections)

Follows hotkeys.py pattern with pynput Windows hooks.
"""
...
# Conversation mode state (runtime only, not persisted)
_current_mode = "default"
_MODES = ["default", "1to1", "continuous"]
...
def set_lua_socket(socket):
    """Set the Lua socket instance for sending mode changes."""
    global _lua_socket
    _lua_socket = socket


def _cycle_mode():
    """Cycle to the next conversation mode."""
    global _current_mode

    idx = _MODES.index(_current_mode)
    idx = (idx + 1) % len(_MODES)
    _current_mode = _MODES[idx]

    print(f"[ModeHotkey] Conversation mode: {_current_mode}")

    # Send mode change to Lua for visual feedback
    if _lua_socket:
        _lua_socket.send_conversation_mode(_current_mode)
...
def start_capture(hotkey='home', check_pause=None):
    """
    Start listening for mode hotkey.

    Args:
        hotkey: Hotkey name ('home', 'end', 'f1'-'f12', etc.)
    ...
    _hotkey_vk = HOTKEY_VK_MAP.get(hotkey.lower(), VK_HOME)
    ...
    _listener.start()
    print(f"[ModeHotkey] Listening for hotkey: {hotkey} (VK={hex(_hotkey_vk)})")
...
def set_hotkey(hotkey):
    ...
    global _hotkey_vk
    new_vk = HOTKEY_VK_MAP.get(hotkey.lower(), VK_HOME)
    if new_vk != _hotkey_vk:
        _hotkey_vk = new_vk
        print(f"[ModeHotkey] Hotkey updated to: {hotkey} (VK={hex(_hotkey_vk)})")
