...
from services.omnivoice_cpp_engine import (
    OUTPUT_SAMPLE_RATE,
    _get_manager,
    clear_voice_prompt,
    is_loaded,
...
class OmniVoiceCppProvider(BaseTTSProvider):
    ...
    def delete_voice(self, voice_id: str) -> bool:
        ...
        try:
            ...
            # Clearing an unloaded worker is already a no-op; do not load the
            # 1.5 GB model just to clear an empty process-local cache.
    ...
    def synthesize_stream(self, text: str, voice_id: str,
        ...
        success, bytes_produced = manager.synthesize_sentence(
            ...
            num_steps=config.get("first_sentence_steps", 24),
    ...
    def synthesize_stream_sentences(self, sentences, voice_id: str,
        ...
        _local_voice_cache = {voice_id: voice_path}

        def _resolve_voice_for_id(vid):
            """Resolve voice paths with per-request caching."""
        ...
        try:
            for item in sentences:
                ...
                sent_voice_path = _resolve_voice_for_id(per_voice_id)
                ...
                success, bytes_produced = manager.synthesize_sentence(
                    ...
                    on_chunk=eq_on_chunk,
                    num_steps=steps,
                    # Pass the current saved base value explicitly. The native
                    # worker's default is captured at process startup, so
                    # omitting this would leave a stale CFG value after the
                    # user changes the setting without changing GPUs.
                    guidance_scale=npc_cfg if npc_cfg is not None else base_cfg,
                    seed=seed,
                )
                ...
                print(f"[OmniVoiceCppTiming] sentence_done idx={sentence_count - 1} "
                      ...
                      f"guidance_scale={npc_cfg if npc_cfg is not None else base_cfg}")
