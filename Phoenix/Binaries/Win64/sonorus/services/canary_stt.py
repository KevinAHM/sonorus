"""
Canary STT Provider (Local ONNX) - Worker Process Architecture

Uses onnx-asr with NVIDIA Canary 180M Flash for local speech recognition.
Models are downloaded from HuggingFace: istupakov/canary-180m-flash-onnx
...
HF_REPO_ID = "istupakov/canary-180m-flash-onnx"

# Local models directory
MODELS_DIR = Path(__file__).resolve().parent.parent / "models" / "canary"
...
# Canary supported 2-letter language codes
SUPPORTED_LANGUAGES = {'en', 'de', 'es', 'fr'}


def _game_lang_to_canary(game_lang: str) -> str:
    """Extract 2-letter prefix from game language code (e.g. 'ES_MX' -> 'es')."""
    code = game_lang.split('_')[0].lower() if game_lang else 'en'
...
def _canary_worker_main(request_queue, response_queue):
    """
    ...
    print(f"[STT/Canary Worker] Starting (PID: {os.getpid()})...")
    ...
    for filename in INT8_MODEL_FILES:
        ...
        if local_path.exists():
            continue
        print(f"[STT/Canary Worker] Downloading {filename}...")
        ...
        print(f"[STT/Canary Worker] Downloaded {filename}")

    # Load model
    print("[STT/Canary Worker] Loading model...")
    ...
    print(f"[STT/Canary Worker] Model loaded in {elapsed:.0f}ms")
    ...
    while True:
        ...
        if req is None:
            print("[STT/Canary Worker] Shutdown signal received")
            break
        ...
        if req_type == "transcribe":
            ...
            try:
                ...
                start = time.time()
                result = model.recognize(tmp_path, language=language, target_language=language)
                elapsed = (time.time() - start) * 1000

                text = result.strip() if isinstance(result, str) else str(result).strip()
                print(f"[STT/Canary Worker] Transcribed: \"{text}\" ({elapsed:.0f}ms)")

                # Retry with silence padding if no speech detected
                if not text:
                    print("[STT/Canary Worker] No speech detected, retrying with 3s silence padding...")
                    ...
                    text = result.strip() if isinstance(result, str) else str(result).strip()
                    print(f"[STT/Canary Worker] Padded retry: \"{text}\" ({elapsed:.0f}ms)")
                ...
                print(f"[STT/Canary Worker] Error: {e}")
                import traceback
                traceback.print_exc()
                response_queue.put({
                    "type": "result",
    ...
    print("[STT/Canary Worker] Exiting")
...
class CanaryProcessManager:
    """Manages the Canary STT worker process from the main process."""
    ...
    def ensure_started(self) -> bool:
        ...
        with self._lock:
            ...
            print("[STT/Canary] Starting worker process...")
            ...
            try:
                while True:
                    ...
                    if msg_type == "ready":
                        self._ready = True
                        print("[STT/Canary] Worker process ready")
                        return True
                    elif msg_type in ("downloading", "loading"):
                        print(f"[STT/Canary] Worker: {resp.get('message', msg_type)}")
                        # Keep waiting — reset timeout
                        continue
                    else:
                        print(f"[STT/Canary] Unexpected worker message: {resp}")
                        self._cleanup()
                        return False
            except Exception as e:
                print(f"[STT/Canary] Worker failed to start: {e}")
                self._cleanup()
                return False
    ...
    def transcribe(self, audio_data: bytes, sample_rate: int = 16000) -> dict:
        ...
        # Resolve language from game settings
        from utils.settings import load_settings
        settings = load_settings()
        game_lang = settings.get('setup', {}).get('language', 'EN_US')
        language = _game_lang_to_canary(game_lang)
        ...
        try:
            ...
            print(f"[STT/Canary] Timeout waiting for transcription result")
            return {
                "success": False,
                "text": "",
                "confidence": 0.0,
    ...
    def shutdown(self):
        ...
        with self._lock:
            self._cleanup()
            print("[STT/Canary] Worker process shut down")
...
def _get_manager() -> CanaryProcessManager:
    """Get or create the process manager singleton."""
    global _process_manager
    if _process_manager is None:
        with _manager_lock:
            if _process_manager is None:
                _process_manager = CanaryProcessManager()
...
def transcribe(audio_data: bytes, sample_rate: int = 16000) -> dict:
    """
    Transcribe audio using Canary ONNX model.
