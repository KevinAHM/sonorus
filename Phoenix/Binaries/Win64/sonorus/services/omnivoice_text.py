...
def _strip_unsupported_tags(text: str) -> str:
    """Normalize and remove bracketed tags that aren't supported audio tags."""
    _TAG_ALIASES = {
        "[laughs]": "[laughter]",
        "[laugh]": "[laughter]",
        "[laughing]": "[laughter]",
    ...
    def repl(match: Match[str]) -> str:
        ...
        full = _TAG_ALIASES.get(full.lower(), full)
        if full in SUPPORTED_AUDIO_TAGS:
            return full
        return ""
    return _BRACKETED_TAG_PATTERN.sub(repl, text).strip()


def preprocess_text(text: str, normalize_exclamation: bool = False) -> str:
    """Normalize text, downcase shouting words (except known abbreviations), strip quotes, collapse whitespace."""
    replacements = [
        ("…", ","),
        ("—", ", "),
        ("–", ", "),
...
def chunk_text_by_time(
    ...
    """Chunk text into ~time-sized pieces."""
    ...
    def flush_chunk() -> None:
        nonlocal chunk_sentences, chunk_secs
        if chunk_sentences:
