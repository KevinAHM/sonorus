...
class OmniVoiceGenerationConfig:
    ...
    block_size: int = 75  # ~3 seconds at 25 Hz frame rate
    first_block_steps: int = 16  # fewer steps when target fits in one block
    # Streaming schedule: list of (num_tokens, scaffold_steps, complete_steps).
    # Each block gets a fresh generation with previous blocks locked in.
    # scaffold_steps: first N steps of a 32-step schedule (partial unmask).
    # complete_steps: fresh schedule to fill remaining masks.
    # complete_steps=0 means scaffold_steps IS the full generation.
    # Remaining tokens after all entries use (None, num_step, 0).
    # e.g. [(25, 8, 8), (25, 8, 12)] = 1s@16total, 1s@20total, rest@32
    streaming_schedule: Optional[list] = None
    streaming_completion_cfg: bool = True  # CFG during completion phase; False saves ~10% TTFA
    # Fast CFG: cache unconditional logits and reuse for N steps.
    # 0=off, 1=first block only, 2=first two blocks, 3=all blocks.
    # For non-streaming, only 3 applies.
    fast_cfg: int = 0
    fast_cfg_refresh: int = 4  # recompute uncond logits every N steps
...
class OmniVoice(PreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        skip_encoder = kwargs.pop("skip_encoder", False)
        ...
        try:
            ...
            if not train_mode:
                ...
                if skip_encoder:
                    # Selectively load only decoder weights from safetensors
                    # — encoder tensors never touch GPU or CPU
                    _encoder_prefixes = (
                        "semantic_model.", "acoustic_encoder.",
                        "encoder_semantic.", "fc.", "fc1.",
                    )
                    from transformers import AutoConfig
                    from safetensors import safe_open
                    _tok_config = AutoConfig.from_pretrained(audio_tokenizer_path)
                    _audio_tok = HiggsAudioV2TokenizerModel(config=_tok_config)
                    _st_path = os.path.join(audio_tokenizer_path, "model.safetensors")
                    if not os.path.isfile(_st_path):
                        import glob as _glob
                        _st_files = _glob.glob(os.path.join(audio_tokenizer_path, "*.safetensors"))
                        _st_path = _st_files[0] if _st_files else None
                    if _st_path:
                        _decoder_sd = {}
                        with safe_open(_st_path, framework="pt", device=str(tokenizer_device)) as _sf:
                            for _key in _sf.keys():
                                if not _key.startswith(_encoder_prefixes):
                                    _decoder_sd[_key] = _sf.get_tensor(_key)
                        _audio_tok.load_state_dict(_decoder_sd, strict=False)
                        del _decoder_sd
                    # Null out encoder modules so they don't occupy even CPU RAM
                    for _name in ("semantic_model", "acoustic_encoder",
                                  "encoder_semantic", "fc", "fc1"):
                        setattr(_audio_tok, _name, None)
                    model.audio_tokenizer = _audio_tok.to(tokenizer_device)
                    del _audio_tok
                else:
                ...
                if compile_model:
                    ...
                    logger.info("LLM backbone compiled with torch.compile")
        finally:
    ...
    def forward(
        ...
        if use_cache:
            ...
            if cache_position is not None:
                llm_kwargs["cache_position"] = cache_position

        llm_outputs = self.llm(**llm_kwargs)
        hidden_states = llm_outputs[0]
        past_kv_out = getattr(llm_outputs, "past_key_values", None)
        ...
        return OmniVoiceModelOutput(
            loss=loss,
            logits=audio_logits,
            past_key_values=past_kv_out if use_cache else None,
        )
    ...
    def generate(
        ...
        **kwargs,
        ...
        if gen_config.blockwise:
            blockwise_results = self._generate_blockwise(full_task, gen_config)
            for idx, res in enumerate(blockwise_results):
                results[idx] = res
        else:
    ...
    def create_voice_clone_prompt(
        ...
        # No Whisper fallback — Sonorus transcribes via its own STT service.
        # Empty string is allowed (tokenizes audio without text alignment).
        if ref_text is None:
            ref_text = ""
    ...
    def _generate_blockwise(
        ...
        """KV-cached generation with global unmasking and early decode.

        Caches the prefix (style+text+ref_audio) KV once, then runs
        global diffusion steps on target tokens only. No block-level
        masking constraint — the model unmasks freely across all
        positions. If ``target_len <= block_size``, stops at
        ``first_block_steps`` for faster TTFA.

        Args:
            task: A :class:`GenerationTask` (batch_size=1 for streaming).
            gen_config: Generation config with block_size, num_step,
                first_block_steps, etc.
        Returns:
            List of generated audio token tensors of shape (C, T).
        """
        assert task.batch_size == 1, "Blockwise KV-cached generation only supports batch_size=1"

        C = self.config.num_audio_codebook
        mask_id = self.config.audio_mask_id
        num_step = gen_config.num_step
        first_block_steps = gen_config.first_block_steps
        target_len = task.target_lens[0]
        block_size = gen_config.block_size

        # Build full input
        inp = self._prepare_inference_inputs(
            task.texts[0], target_len, task.ref_texts[0],
            task.ref_audio_tokens[0], task.langs[0], task.instructs[0],
            gen_config.denoise,
        )
        full_ids = inp["input_ids"]
        full_audio_mask = inp["audio_mask"]
        total_len = full_ids.size(2)
        prefix_len = total_len - target_len
        target_audio_mask = full_audio_mask[:, prefix_len:]

        # --- Cache warmup: full forward, then crop to prefix ---
        full_attn = torch.ones(1, 1, total_len, total_len, dtype=torch.bool, device=self.device)
        prefix_cache = self(
            input_ids=full_ids, audio_mask=full_audio_mask,
            use_cache=True, attention_mask=full_attn,
        ).past_key_values
        prefix_cache.crop(prefix_len)
        del full_attn

        # --- Pre-allocate ---
        tokens = torch.full((1, C, target_len), mask_id, dtype=torch.long, device=self.device)
        layer_ids = torch.arange(C, device=self.device).view(1, -1, 1)
        cache_pos = torch.arange(prefix_len, total_len, device=self.device)
        cond_attn = torch.ones(1, 1, target_len, total_len, dtype=torch.bool, device=self.device)
        uncond_attn = torch.ones(1, 1, target_len, target_len, dtype=torch.bool, device=self.device)

        # Early exit: if target fits in one block, use fewer steps
        effective_steps = first_block_steps if target_len <= block_size else num_step

        # Diffusion schedule
        total_mask = target_len * C
        timesteps = _get_time_steps(
            t_start=0.0, t_end=1.0, num_step=effective_steps + 1, t_shift=gen_config.t_shift,
        ).tolist()
        rem = total_mask
        schedule = []
        for s in range(effective_steps):
            n = rem if s == effective_steps - 1 else min(
                math.ceil(total_mask * (timesteps[s + 1] - timesteps[s])), rem
            )
            schedule.append(int(n))
            rem -= int(n)

        # --- Fast CFG state for non-streaming ---
        use_fast_cfg = gen_config.fast_cfg == 3
        refresh = gen_config.fast_cfg_refresh
        _cached_u_logits = None

        # --- Global generation loop ---
        for step in range(effective_steps):
            k = schedule[step]
            if k <= 0:
                continue

            # Cond: target tokens with cached prefix KV
            c_logits = self(
                input_ids=tokens, audio_mask=target_audio_mask,
                use_cache=True, past_key_values=prefix_cache,
                cache_position=cache_pos, attention_mask=cond_attn,
            ).logits.to(torch.float32)
            prefix_cache.crop(prefix_len)

            # Uncond: target tokens only
            if use_fast_cfg and _cached_u_logits is not None and step % refresh != 0:
                u_logits = _cached_u_logits
            else:
                u_logits = self(
                    input_ids=tokens, audio_mask=target_audio_mask,
                ...
                if use_fast_cfg:
                    _cached_u_logits = u_logits

            # Score and unmask globally (no block constraint)
            pred_tokens, scores = self._predict_tokens_with_scoring(
                c_logits, u_logits, gen_config,
            )
            scores = scores - (layer_ids * gen_config.layer_penalty_factor)
            ...
            tokens.copy_(flat.view_as(tokens))
    ...
    def generate_streaming(
        ...
        **kwargs,
        ...
        """Generate audio progressively and yield decoded chunks.

        Each block in the schedule gets a fresh generation with previous
        blocks' tokens locked in. Uses scaffold (partial 32-step schedule)
        + completion (fresh schedule for remaining masks) per block.
        Decoded with higgs-style overlap + Hamming crossfade.

        ``streaming_schedule``: list of ``(num_tokens, scaffold_steps,
        complete_steps)`` tuples. ``complete_steps=0`` means run
        ``scaffold_steps`` as a full generation. Remaining tokens after
        all entries use ``(None, num_step, 0)``.

        e.g. ``[(25, 8, 8), (25, 8, 12)]`` = 1s@16 total, 1s@20 total,
        rest@32.

        Yields:
            ``(block_index, audio_tensor)`` where audio_tensor is ``(1, T)``.
        """
        from omnivoice.utils.audio import trim_leading_silence

        if self.audio_tokenizer is None or self.text_tokenizer is None:
            raise RuntimeError("Model not loaded with tokenizers.")
        ...
        full_task = self._preprocess_all(
            text=text, language=language, ref_text=ref_text,
            ref_audio=ref_audio, voice_clone_prompt=voice_clone_prompt,
            instruct=instruct, preprocess_prompt=gen_config.preprocess_prompt,
            speed=speed, duration=duration,
        )
        assert full_task.batch_size == 1, "Streaming only supports single text input"

        C = self.config.num_audio_codebook
        mask_id = self.config.audio_mask_id
        target_len = full_task.target_lens[0]

        inp = self._prepare_inference_inputs(
            full_task.texts[0], target_len, full_task.ref_texts[0],
            full_task.ref_audio_tokens[0], full_task.langs[0],
            full_task.instructs[0], gen_config.denoise,
        )
        full_ids = inp["input_ids"]
        full_audio_mask = inp["audio_mask"]
        total_len = full_ids.size(2)
        ...
        # Build schedule
        if gen_config.streaming_schedule is not None:
            schedule = list(gen_config.streaming_schedule)
        else:
            schedule = [(gen_config.block_size, gen_config.first_block_steps,
                         gen_config.num_step - gen_config.first_block_steps)]

        # Append final block for remaining tokens
        scheduled_tokens = sum(s[0] for s in schedule if s[0] is not None)
        if scheduled_tokens < target_len:
            schedule.append((None, gen_config.num_step, 0))

        locked = {}  # tok_start -> (C, num_tokens) tensor

        def _make_tokens():
            """Fresh target tokens with locked blocks pre-filled."""
            t = torch.full((1, C, target_len), mask_id, dtype=torch.long, device=self.device)
            for start, toks in locked.items():
                t[0, :, start:start + toks.size(1)] = toks
            return t

        def _warmup(tokens):
            """Build prefix KV cache including current token state."""
            wids = full_ids.clone()
            wids[0, :, prefix_len:] = tokens[0]
            fa = torch.ones(1, 1, total_len, total_len, dtype=torch.bool, device=self.device)
            pc = self(input_ids=wids, audio_mask=full_audio_mask,
                      use_cache=True, attention_mask=fa).past_key_values
            pc.crop(prefix_len)
            return pc

        # Fast CFG state: cache uncond logits to skip uncond forward passes
        _ucfg = {"logits": None, "step": 0}

        def _step(tokens, pc, k, use_cfg=True, fast_cfg=False):
            """One global diffusion step."""
            c_logits = self(
                input_ids=tokens, audio_mask=target_audio_mask,
                use_cache=True, past_key_values=pc,
                cache_position=cache_pos, attention_mask=cond_attn,
            ).logits.to(torch.float32)
            pc.crop(prefix_len)

            if use_cfg and gen_config.guidance_scale != 0:
                refresh = gen_config.fast_cfg_refresh
                if fast_cfg and _ucfg["logits"] is not None and _ucfg["step"] % refresh != 0:
                    u_logits = _ucfg["logits"]
                else:
                    u_logits = self(
                        input_ids=tokens, audio_mask=target_audio_mask,
                        attention_mask=uncond_attn,
                    ...
                    if fast_cfg:
                        _ucfg["logits"] = u_logits
                _ucfg["step"] += 1
            else:
                u_logits = c_logits  # no-op, _predict_tokens_with_scoring handles scale=0

            pred_tokens, scores = self._predict_tokens_with_scoring(
                c_logits, u_logits if use_cfg else None, gen_config,
            )
            scores = scores - (layer_ids * gen_config.layer_penalty_factor)
            if gen_config.position_temperature > 0.0:
                scores = _gumbel_sample(scores, gen_config.position_temperature)
        ...
        def _run_block(scaffold_steps, complete_steps, use_cfg=True, fast_cfg=False):
            """Run one block: scaffold + completion. Returns token state."""
            # Reset fast CFG cache for each block
            _ucfg["logits"] = None
            _ucfg["step"] = 0

            tokens = _make_tokens()
            current_masks = (tokens == mask_id).sum().item()

            if complete_steps > 0:
                # Scaffold: first N steps of 32-step schedule pacing (with CFG)
                pc = _warmup(tokens)
                ts32 = _get_time_steps(0.0, 1.0, 33, gen_config.t_shift).tolist()
                r = current_masks
                sched32 = []
                for s in range(32):
                    n = r if s == 31 else min(math.ceil(current_masks * (ts32[s + 1] - ts32[s])), r)
                    sched32.append(int(n))
                    r -= int(n)
                for s in range(scaffold_steps):
                    _step(tokens, pc, sched32[s], use_cfg=use_cfg, fast_cfg=fast_cfg)

                # Completion: fresh schedule for remaining masks
                # Reuse scaffold's prefix cache (still valid after crop)
                completion_cfg = use_cfg and gen_config.streaming_completion_cfg
                _ucfg["logits"] = None  # reset cache for completion phase
                _ucfg["step"] = 0
                remaining = (tokens == mask_id).sum().item()
                pc2 = pc
                ts_c = _get_time_steps(0.0, 1.0, complete_steps + 1, gen_config.t_shift).tolist()
                r = remaining
                sched_c = []
                for s in range(complete_steps):
                    n = r if s == complete_steps - 1 else min(math.ceil(remaining * (ts_c[s + 1] - ts_c[s])), r)
                    sched_c.append(int(n))
                    r -= int(n)
                for s in range(complete_steps):
                    _step(tokens, pc2, sched_c[s], use_cfg=completion_cfg, fast_cfg=fast_cfg)
            else:
                # Full run
                pc = _warmup(tokens)
                ts = _get_time_steps(0.0, 1.0, scaffold_steps + 1, gen_config.t_shift).tolist()
                r = current_masks
                sched = []
                for s in range(scaffold_steps):
                    n = r if s == scaffold_steps - 1 else min(math.ceil(current_masks * (ts[s + 1] - ts[s])), r)
                    sched.append(int(n))
                    r -= int(n)
                for s in range(scaffold_steps):
                    _step(tokens, pc, sched[s], use_cfg=use_cfg, fast_cfg=fast_cfg)

            return tokens[0]

        # Generate and yield blocks
        tok_cursor = 0
        fade_out_audio = None
        overlap_toks = 3  # codec crossfade context

        for block_idx, block_params in enumerate(schedule):
            if len(block_params) == 4:
                num_tokens, scaffold_steps, complete_steps, use_cfg = block_params
            else:
                num_tokens, scaffold_steps, complete_steps = block_params
                use_cfg = True
            tok_end = min(tok_cursor + num_tokens, target_len) if num_tokens is not None else target_len
            if tok_cursor >= target_len:
                break

            # Determine if fast_cfg applies to this block
            # Only apply when using a real streaming schedule (not fallback single-block)
            fc = gen_config.fast_cfg if gen_config.streaming_schedule is not None else 0
            use_fast_cfg = fc == 3 or (fc >= 1 and block_idx < fc)

            tokens = _run_block(scaffold_steps, complete_steps, use_cfg=use_cfg, fast_cfg=use_fast_cfg)
            block_tokens = tokens[:, tok_cursor:tok_end].clone()
            locked[tok_cursor] = block_tokens

            is_last = (tok_end >= target_len)

            # Decode with overlap on both sides
            dec_start = max(0, tok_cursor - overlap_toks)
            dec_end = min(tok_end + (0 if is_last else overlap_toks), target_len)
            dec_tokens = tokens[:, dec_start:dec_end]
            raw = (
                self.audio_tokenizer.decode(
                    dec_tokens.to(tokenizer_device).unsqueeze(0)
                ).audio_values[0].cpu()
            )
            spt = raw.shape[-1] / (dec_end - dec_start)

            # Slice nominal block audio
            nom_start = int((tok_cursor - dec_start) * spt)
            nom_end = int((tok_end - dec_start) * spt)
            tts_speech = raw[0, nom_start:nom_end]

            # Hamming crossfade with previous fade_out
            if fade_out_audio is not None:
                hamming_len = min(2 * len(fade_out_audio), int(spt))
                if hamming_len > 1:
                    hamming = torch.hamming_window(hamming_len)
                    fade_overlap = hamming_len // 2
                    if (fade_overlap > 0
                            and fade_overlap <= len(tts_speech)
                            and fade_overlap <= len(fade_out_audio)):
                        tts_speech[:fade_overlap] = (
                            tts_speech[:fade_overlap] * hamming[:fade_overlap]
                            + fade_out_audio[:fade_overlap] * hamming[fade_overlap:]
                        )

            # Save tail beyond nominal as fade_out
            fade_out_audio = raw[0, nom_end:] if not is_last else None

            chunk = tts_speech.unsqueeze(0)

            yield block_idx, chunk

            tok_cursor = tok_end

    def _predict_tokens_with_scoring(self, c_logits, u_logits, gen_config):
        if u_logits is not None and gen_config.guidance_scale != 0:
            c_log_probs = F.log_softmax(c_logits, dim=-1)
            u_log_probs = F.log_softmax(u_logits, dim=-1)
            log_probs = torch.log_softmax(
                u_log_probs + gen_config.guidance_scale * (c_log_probs - u_log_probs),
                dim=-1,
            )
        else:
