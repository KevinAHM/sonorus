...
from cognis.utils import normalize_embedding_model_id, qdrant_uuid

logger = logging.getLogger(__name__)


def _ensure_pywin32_paths() -> None:
    """Make pywin32 importable in Sonorus's isolated embedded Python."""
    if os.name != "nt":
        return

    site_packages = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    win32_dir = os.path.join(site_packages, "win32")
    win32_lib_dir = os.path.join(win32_dir, "lib")
    pywin32_system32 = os.path.join(site_packages, "pywin32_system32")

    for path in (win32_dir, win32_lib_dir):
...
class QdrantLocalStore:
    ...
    def connect(self) -> None:
        ...
        self._migrate_embedding_model_payloads()
    ...
    def _ensure_indexes(self, collection: str) -> None:
        ...
        keyword_fields = [
            ...
            "status", "category", "type", "embedding_model", "embedding_model_key",
        ]
    ...
    def _migrate_embedding_model_payloads(self) -> None:
        """Stamp legacy vector payloads with raw and normalized embedding model IDs."""
            ...
            try:
                while True:
                    ...
                    for point in points:
                        ...
                        embedding_model = payload.get("embedding_model") or self._embedding_model
                        embedding_model_key = normalize_embedding_model_id(embedding_model)
                        updates = {}
                        if not payload.get("embedding_model"):
                            updates["embedding_model"] = embedding_model
                        if payload.get("embedding_model_key") != embedding_model_key:
                            updates["embedding_model_key"] = embedding_model_key
                        if updates:
                ...
                if updated:
                    logger.info(
                        "Stamped %d legacy Qdrant payloads in %s with normalized embedding model IDs",
                        updated,
                        collection,
                    )
            except Exception as exc:
                logger.warning("Embedding model payload migration skipped for %s: %s", collection, exc)
    ...
    def upsert_immediate_message(
        ...
        payload = {
            ...
            "embedding_model_key": normalize_embedding_model_id(embedding_model or self._embedding_model),
        }
        self._client.upsert(
            collection_name=self._collection_small,
    ...
    def _build_filter(
        ...
        if embedding_model:
            conditions.append(FieldCondition(
                ...
                match=MatchValue(value=normalize_embedding_model_id(embedding_model)),
            ))
