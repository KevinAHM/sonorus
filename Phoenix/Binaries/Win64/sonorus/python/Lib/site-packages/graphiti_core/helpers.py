...
def parse_jsonish_dict(value: Any, field_name: str = 'value') -> dict[str, Any]:
    """Parse dict-like values that may arrive as JSON or Python-literal strings.

    Ladybug has been observed returning text like "{'house': 'Slytherin', 'role': None}"
    for Kuzu string properties that Graphiti originally wrote as JSON. Graphiti's
    original strict json.loads() then explodes during read paths. We accept either
    strict JSON or a Python literal dict here and fall back to an empty dict for
    anything else.
    """
    if value is None:
        return {}
    ...
    if not isinstance(value, str):
        logger.warning('Unexpected %s type %s; using empty dict', field_name, type(value).__name__)
        return {}
    ...
    try:
        ...
        try:
            ...
            logger.warning('Failed to parse %s as JSON or Python literal: %r', field_name, value)
            return {}

    if isinstance(parsed, dict):
        return parsed

    logger.warning('Parsed %s as %s instead of dict; using empty dict', field_name, type(parsed).__name__)
    return {}
