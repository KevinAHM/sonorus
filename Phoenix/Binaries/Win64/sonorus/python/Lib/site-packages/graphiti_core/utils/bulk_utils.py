...
async def add_nodes_and_edges_bulk(
    ...
    # Checkpoint WAL after bulk writes so committed data is flushed to the main
    # DB file. Without this, the WAL accumulates dirty records for the entire
    # duration of add_episode (5-10+ min) and any crash corrupts it.
    if driver.provider == GraphProvider.KUZU:
        try:
            await driver.execute_query("CHECKPOINT;")
        except Exception:
            logging.getLogger(__name__).warning('WAL checkpoint after bulk write failed', exc_info=True)
