...
RegisterHook("/Script/Engine.PlayerController:ClientRestart", function(Context, NewPawn)
    ...
    Cache.ClearObjects()
    Cache.InvalidateStatic()
    -- Delay slightly to ensure UIManager is ready
    ExecuteWithDelay(1000, UpdatePlayerInfo)
end)
...
LoopAsync(2000, function()
    ...
    -- Continue searching (async check means we check cached result next iteration)
    return false
end)
...
LoopAsync(3000, function()
    ...
    return true  -- Stop after one run
end)
...
LoopAsync(2000, function()
    return TryRegisterHooksForModActors()
end)
