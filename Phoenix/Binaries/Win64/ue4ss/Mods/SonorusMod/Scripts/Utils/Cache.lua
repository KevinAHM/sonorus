...
---Clear entity cache for a type (forces re-FindAllOf on next access)
---@param entityType string e.g., "NPC"
function Cache.ClearEntities(entityType)
    if store.entities[entityType] then
        local count = #store.entities[entityType].list
        store.entities[entityType].list = {}
        store.entities[entityType].set = {}
        store.entities[entityType].initialized = false
        -- Note: hookRegistered stays true - hooks persist
        print(string.format("[Cache] Cleared %s entity cache (%d entries), will re-init on next access", entityType, count))
    end
end

---Clear all entity caches (forces re-FindAllOf on next access)
function Cache.ClearAllEntities()
    for entityType, cache in pairs(store.entities) do
        local count = #cache.list
        cache.list = {}
        cache.set = {}
        cache.initialized = false
        print(string.format("[Cache] Cleared %s entity cache (%d entries)", entityType, count))
    end
end
